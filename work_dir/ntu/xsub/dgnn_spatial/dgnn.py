...
class Model(nn.Module):
    ...
    def forward(self, fv, fe): # fv : batch_joint_data, fe : batch_bone_data
        N, C, T, V_node, M = fv.shape # examples (N), channels (C), frames (T), nodes (V), persons (M)
        _, _, _, V_edge, _ = fe.shape

        # Preprocessing
        fv = fv.permute(0, 4, 3, 1, 2).contiguous().view(N, M * V_node * C, T) # N, M*V*C, T
        fv = self.data_bn_v(fv) # batch norm
        fv = fv.view(N, M, V_node, C, T).permute(0, 1, 3, 4, 2).contiguous().view(N * M, C, T, V_node) # N*M, C, T, V

        fe = fe.permute(0, 4, 3, 1, 2).contiguous().view(N, M * V_edge * C, T) # N, M*V*C, T
        fe = self.data_bn_e(fe) # batch norm
        fe = fe.view(N, M, V_edge, C, T).permute(0, 1, 3, 4, 2).contiguous().view(N * M, C, T, V_edge) # N*M, C, T, V
